 ...
 * @return array
 */
function drillingservices_body_classes( $classes ) {
...
add_filter( 'body_class', 'drillingservices_body_classes' );
...
if (!function_exists('drillingservices_str_replace_assoc')) {

    /**
     * drillingservices_str_replace_assoc
     * @param  array $replace
     * @param  array $subject
     * @return array
     */
    function drillingservices_str_replace_assoc(array $replace, $subject) {
 ...
 */
function drillingservices_add_to_cart_fragment( $fragments ) {
	...
	<a class="nav-link cart-icon-wrap header-cart" href="javascript:void(0);" role="button" data-bs-toggle="collapse" data-bs-target="#Cart">
		...
		?>	
			<span><?php echo esc_html__('0','drilling-services'); ?></span>
		<?php
		}
		?>
	</a>
...
add_filter( 'woocommerce_add_to_cart_fragments', 'drillingservices_add_to_cart_fragment' );
...
if ( ! function_exists( 'drillingservices_breadcrumb_title' ) ) {
	function drillingservices_breadcrumb_title() {
		...
		elseif ( is_day() ) : 
			...
			printf( __( 'Daily Archives: %s', 'drilling-services' ), get_the_date() );
		
		elseif ( is_month() ) :
		
			printf( __( 'Monthly Archives: %s', 'drilling-services' ), (get_the_date( 'F Y' ) ));
			
		elseif ( is_year() ) :
		
			printf( __( 'Yearly Archives: %s', 'drilling-services' ), (get_the_date( 'Y' ) ) );
			
		elseif ( is_category() ) :
		
			printf( __( 'Category Archives: %s', 'drilling-services' ), (single_cat_title( '', false ) ));

		elseif ( is_tag() ) :
		
			printf( __( 'Tag Archives: %s', 'drilling-services' ), (single_tag_title( '', false ) ));
			
		elseif ( is_404() ) :

			printf( __( 'Error 404', 'drilling-services' ));
			
		elseif ( is_author() ) :
		
			printf( __( 'Author: %s', 'drilling-services' ), (get_the_author( '', false ) ));		
...
function drillingservices_breadcrumbs() {
	
	$showOnHome	= esc_html__('1','drilling-services'); 	// 1 - Show breadcrumbs on the homepage, 0 - don't show
	$delimiter 	= '';   // Delimiter between breadcrumb
	$home 		= esc_html__('Home','drilling-services'); 	// Text for the 'Home' link
	$showCurrent= esc_html__('1','drilling-services'); // Current post/page title in breadcrumb in use 1, Use 0 for don't show
    ...
    if ( is_category() ) 
	{
		...
		echo $before . esc_html__('Archive by category','drilling-services').' "' . esc_html(single_cat_title('', false)) . '"' .$after;
		
	} 
	
	elseif ( is_search() ) 
	{
		echo $before . esc_html__('Search results for','drilling-services').' "' . esc_html(get_search_query()) . '"' . $after;
	} 
	...
	elseif ( is_tag() ) 
	{
		echo $before . esc_html__('Posts tagged ','drilling-services').' "' . esc_html(single_tag_title('', false)) . '"' . $after;
	} 
	
	elseif ( is_author() ) {
		global $author;
		$userdata = get_userdata($author);
		echo $before . esc_html__('Articles posted by ','drilling-services').'' . $userdata->display_name . $after;
	} 
	
	elseif ( is_404() ) {
		echo $before . esc_html__('Error 404 ','drilling-services'). $after;
    }
	
    if ( get_query_var('paged') ) {
		if ( is_category() || is_day() || is_month() || is_year() || is_search() || is_tag() || is_author() ) echo '';
		echo ' ( ' . esc_html__('Page','drilling-services') . '' . esc_html(get_query_var('paged')). ' )';
		if ( is_category() || is_day() || is_month() || is_year() || is_search() || is_tag() || is_author() ) echo '';
    }
...
add_action( 'wp_ajax_drillingservices_dismissed_notice_handler', 'drillingservices_ajax_notice_handler' );

/**
 * AJAX handler to store the state of dismissible notices.
 */
function drillingservices_ajax_notice_handler() {
    if ( isset( $_POST['type'] ) ) {
...
add_action( 'wp_ajax_install_act_plugin', 'drillingservices_admin_install_plugin' );

function drillingservices_admin_install_plugin() {
    /**
     * Install Plugin.
     */
